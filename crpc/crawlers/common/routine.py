...
def spout_listing(site):
    ...
    now = datetime.utcnow()
    if hasattr(m, 'Event'):
        return m.Event.objects(Q(urgent=True) & (Q(events_begin__lte=now) | Q(events_begin__exists=False)) & (Q(events_end__gte=now) | Q(events_end__exists=False)) & (Q(is_leaf=True) | Q(is_leaf__exists=False))).order_by('-update_time').timeout(False)
...
def spout_listing_update(site):
    ...
    now = datetime.utcnow()
    if hasattr(m, 'Event'):
        return m.Event.objects(Q(urgent=False) & (Q(events_begin__lte=now) | Q(events_begin__exists=False)) & (Q(events_end__gte=now) | Q(events_end__exists=False)) & (Q(is_leaf=True) | Q(is_leaf__exists=False))).order_by('-update_time').timeout(False)
    if hasattr(m, 'Category'):
        return m.Category.objects(is_leaf=True).order_by('-update_time').timeout(False)
