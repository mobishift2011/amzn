...
def second_category():
    caturl = 'http://www.overstock.com'
    tree = iurl_otree(caturl)

    nodes = tree.xpath('//div[@id="newo-hd"]/div[@id="content-top-rail"]//li[@class="top alldeptsmenu"]/div')
    if not nodes: 
        print 'Url can not be parsed ', caturl
        return
    ...
    mainCategory = dict(zip(names, links))
    print mainCategory
    exit()
