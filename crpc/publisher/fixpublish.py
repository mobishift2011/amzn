...
class DupeFixer:
    ...
    def fix_products_in_file(self, filename):
        pattern = re.compile('publishing product (?P<site>\w+):(?P<key>\w+) failed')
    ...
    def fix_events_in_file(self, filename):
        pattern = re.compile('publishing event (?P<site>\w+):(?P<key>\w+) failed')
        with open(filename) as f:
            for line in f:
                m = pattern.search(line)
                if m:
...
if __name__ == '__main__':
    ...
    if options.file:
        ...
        fixer.fix_product(options.site, options.prod)
    elif options.site and options.ev:
        fixer.fix_event(options.site, options.ev)
