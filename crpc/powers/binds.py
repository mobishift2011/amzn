...
def single_image_crawling(sender, **kwargs):
    ...
    if site and key and doctype.capitalize() in ('Event', 'Product'):
        try:
            ...
            logger.error('Error single image: {0}. {1}'.format(e, traceback.format_exc()))
    else:
...
def batch_image_crawling(sender, **kwargs):
    ...
    if site and doctype:
        try:
            ...
            logger.error('Error batch image: {0}. {1}'.format(e, traceback.format_exc()))
    else:
...
def batch_text_extract(sender, **kwargs):
    ...
    try:
        ...
        logger.error('Error text: {0}. {1}'.format(e, traceback.format_exc()))
    finally:
