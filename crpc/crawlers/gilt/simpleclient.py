    ...
    def check_onsale_product(self, id, url):
        ...
        if '/home/sale' in url or '/sale/home' in url: # home
            ...
            if listprice and '-' in listprice:
                print 'gilt product[{0}] listprice {1} vs {2}'.format(url, prd.listprice, listprice)
            ...
            if '-' in price:
                print 'gilt product[{0}] price {1} vs {2}'.format(url, prd.price, price)
            elif float(prd.price.replace('$', '').replace('Gilt', '').strip()) != float(price):
                print 'gilt product[{0}] price error: [{1}, {2}]'.format(url, prd.price.replace('$', '').replace('Gilt', '').strip(), price)
            ...
            if '-' in price:
                print 'gilt product[{0}] price {1} vs {2}'.format(url, prd.price, price)
            elif float(prd.price.replace('$', '').strip()) != float(price):
