...
def spout_listing_update(site):
    ...
    if hasattr(m, 'Event'):
        ...
        return m.Event.objects(Q(urgent=False) & (Q(events_begin__lt=now) | Q(events_begin__exists=False)) & (Q(events_end__gt=now) | Q(events_end__exists=False))).order_by('-update_time').timeout(False)
